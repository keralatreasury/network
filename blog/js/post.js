    ...
    async function renderPostPage(post, comments) {
        ...
        // Show featured image only if exists
        if(featuredImg) {
        ...
        // Updated share button to copy URL with post ID
        document.getElementById("shareButton").addEventListener("click", () => {
            const shareUrl = getCurrentPageUrl(post.id);
            navigator.clipboard.writeText(shareUrl).then(() => {
                showToastMessage("📋 Blog URL copied! (Includes post ID)");
            }).catch(() => {
